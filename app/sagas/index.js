...
function * authorize ({username, password, isRegistering}) {
    ...
    yield put({type: SENDING_REQUEST, payload: true})
    ...
    try {
        ...
        yield put({type: SENDING_REQUEST, payload: false})
    }
}
...
export function * loginFlow () {
    ...
    while (true) {
        ...
        if (winner.auth) {
            ...
            yield put({type: SET_AUTH, payload: true}) // User is logged in (authorized)
            console.log('auth one')

            yield put({type: CHANGE_FORM, payload: {username: '', password: ''} }) // Clear form
            ...
            yield put({type: SET_AUTH, payload: false}) // User is not logged in (not authorized)
...
export function * registerFlow () {
    while (true) {
        ...
        if (wasSuccessful) {
            yield put({type: SET_AUTH, payload: true}) // User is logged in (authorized) after being registered
            yield put({type: CHANGE_FORM, payload: {username: '', password: ''}}) // Clear form
            forwardTo('/dashboard') // Go to dashboard page
        }
    }
